...
from fanjiang.data import create_test_loader, create_train_loader
...
from .defaults import default_writers
...
class EMA:
    def __init__(self, model, decay=0.999, warmup_iters=1000):
...
class AdversarialTrainer(TrainerBase):
    def __init__(self, cfg):
        ...
        self.with_adv = cfg.MODEL.WITH_ADV
        self.with_ema = cfg.MODEL.WITH_EMA
        self.with_amp = cfg.MODEL.WITH_AMP
        self.with_sty = cfg.MODEL.WITH_STY
        self.teach_forcing = cfg.SOLVER.TEACHER_FORCING.ENABLED
        ...
        self.clip_grad = cfg.SOLVER.CLIP_GRADIENTS.ENABLED
        self.clip_norm = cfg.SOLVER.CLIP_GRADIENTS.CLIP_VALUE

        self.cfg = cfg

        generator = self.build_models(cfg.MODEL.GENERATOR).to(self.device)

        if self.with_ema:
            self.ema = EMA(generator, warmup_iters=self.warmup_iters)

        optimizer = build_optimizer(cfg, generator)
        generator = create_ddp_model(generator, broadcast_buffers=False)
        ...
        if self.with_adv:
            discriminator = self.build_models(cfg.MODEL.DISCRIMINATOR).to(self.device)
            optimizer_D = build_optimizer(cfg, discriminator)
            discriminator = create_ddp_model(discriminator, broadcast_buffers=False)
            scheduler_D = build_lr_scheduler(cfg, optimizer_D)
    ...
    def adjust_sampling(self):
        ...
        k = self.cfg.SOLVER.TEACHER_FORCING.ITERS
    ...
    def run_step(self):
        ...
        if self.teach_forcing:
            info.update(self.adjust_sampling())

        with autocast(enabled=self.with_amp):
            outputs = self.generator(inputs, info=info)
            loss_G.update(outputs["loss"])

        if self.with_sty:
            fake_img = outputs["output"]
            real_img = outputs["target"]
            loss_G["loss_VGG"] = self.criterions["VGG"](fake_img, real_img)

        if self.with_adv:
            fake_img = outputs["output_cond"]
            fake_feats = self.discriminator(fake_img)
            loss_G["loss_G_gan"] = self.criterions["GAN"](fake_feats, True, is_disc=False)

        loss = sum(loss_G.values())
        self.grad_scaler.scale(loss).backward()

        if self.clip_grad:
            self.grad_scaler.unscale_(self.optimizer)
            torch.nn.utils.clip_grad_norm_(self.generator.parameters(), self.clip_norm)
        ...
        if self.with_ema:
            self.ema.update(self.generator, self.iter)
        ...
        if self.with_adv:
            ...
            outputs = self.generator(inputs, info=info)
            fake_img = outputs["output_cond"]
            real_img = outputs["target_cond"]

            fake_feats = self.discriminator(fake_img)
            loss_D["loss_D_fake"] = self.criterions["GAN"](fake_feats, False, is_disc=True)

            if self.with_reg & (self.iter % self.criterions["R1"].interval == 0):
                real_feats = self.discriminator(real_img.requires_grad_())
                loss_D["loss_D_r1"] = self.criterions["R1"](real_img, real_feats)
            else:
                real_feats = self.discriminator(real_img)

            loss_D["loss_D_real"] = self.criterions["GAN"](real_feats, True, is_disc=True)
        ...
        SimpleTrainer.write_metrics(loss_G, data_time)
