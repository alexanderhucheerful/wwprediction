...
from fanjiang.layers import AdaIN, Conv2d, get_norm, DropPath, to_2tuple, trunc_normal_
from fanjiang.layers import SEAttention
...
def window_partition_bk(x, window_size):
    """
    Args:
        x: (B, H, W, C)
...
def window_reverse_bk(windows, window_size, H, W):
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
...
class StyleSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, style_dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU):
        super().__init__()
        self.dim = dim
...
class SwinTransformerBlock_bk(nn.Module):
    r""" Swin Transformer Block.
...
class WindowAttention_kv(nn.Module):
    ...
    def __init__(
            self, dim, dim_out, window_size, num_heads,
...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def __init__(self, in_dim, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        ...
        self.proj = nn.Linear(in_dim + dim, dim, bias=False)
        self.norm1 = norm_layer(dim)
    ...
    def forward(self, x, m):
        ...
        m_windows = m_windows.flatten(1, 2)

        # W-MSA/SW-MSA
        # x_windows = torch.cat([x_windows, m_windows], dim=-1)
        x_windows = self.proj(torch.cat([x_windows, m_windows], dim=-1))
...
class CubeEmbed(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(mid_channels, mid_channels * 2, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(mid_channels * 2, mid_channels, kernel_size=3, padding=1)
        ...
        # self.se = SEAttention(mid_channels, reduction=8)
    ...
    def forward(self, x):
        ...
        x = rearrange(x, '(n t) c h w -> n c t h w', n=bs)
        x = self.proj(x)
...
class SwinEncoder(SwinTransformer):
    def __init__(
        ...
        for i in self.out_indices:
            ...
            proj_layer = nn.Linear(dim, dims[i], bias=False)
